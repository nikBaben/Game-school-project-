import pygame
from styles import skin_1_anim,anim_skin2,anim_second_ship,img_skin3
...
class Skin_3():
    def __init__(self,screen): 
       ...
       # self.y = 200
        #self.x = 500
        self.img = pygame.transform.scale((img_skin3),(220, 250)).convert_alpha()
        self.rect = self.img.get_rect()
        self.screen_rect = screen.get_rect()
        self.frame = 0  # Номер кадра в списке anim, изначально равне 0
        self.last_update = pygame.time.get_ticks()  # Получаем последний кадр игры
        self.frame_rate = 70
        self.rect.centerx = self.screen_rect.centerx + 200
        self.rect.bottom = self.screen_rect.bottom - 0
        self.x = float(self.rect.centerx)
        self.y = float(self.rect.bottom)
